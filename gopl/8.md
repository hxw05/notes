...
以上四种goroutine之间共享的变量只有`net.Conn`（从main中传到handleConn中）和通道，而它们都是并发安全的（concurrency safe）。并发不安全的map被限制在broadcaster goroutine中，外部无法访问到，这也是一种保护非并发安全量的方法：定义在local scope使其完全无法被外部读取。