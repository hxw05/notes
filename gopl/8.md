...
在这个例子中有四种goroutine：
- main goroutine：用于监听Web请求，将进来的TCP连接移交给handleConn函数处理
- handleConn goroutine：用于对一个TCP连接进行处理，为其创建进入聊天室的通道（记为client，这是一个单向的 `<-chan string`）、做一些准备工作（例如广播一则“XXX进入了聊天室”的信息）、为其启动一个clientWriter以不断写入收到的消息并使用scanner读取客户端发送的信息。
- clientWriter goroutine：借助`fmt.Fprintf`来向TCP连接写入信息
- broadcaster goroutine：维护一个clients map，负责接受信息并将这些信息广播给所有连接的客户端，并处理客户端的加入和退出操作

起核心作用的是broadcaster goroutine，它维护了三个不同的通道
- entering：用于表示一个client的加入
- leaving：用于表示一个client的退出
- messages：用于表示聊天室中发送的信息

broadcaster goroutine的内部创建了一个用于存放client的集合，当一个客户端连接上聊天服务器，就会被立即加入到client集合中。

广播的实现方法是broadcaster每接收到一个信息，就将这个信息统一“转发”所有集合中的客户端。虽然通道是无缓存的，但其只有broadcaster这一个发送的目标，而broadcaster对通道中的数据的消费方式就是将其广播出去，这就是broadcaster的主要作用。
```go{14-16}
...
当一个客户端退出聊天室以后，需要将其通道关闭，这是一个自然的步骤。在当前的方案中，关闭通道的一个重要作用是使clientWriter goroutine可以自然结束（for循环终止）。

以上四种goroutine之间共享的变量只有`net.Conn`（从main中传到handleConn中）和通道，而它们都是并发安全的（concurrency safe）。并发不安全的map被限制在broadcaster goroutine中，外部无法访问到，这也是一种保护非并发安全量的方法：定义在local scope使其完全无法被外部读取（在爬虫的例子中也提到过）。