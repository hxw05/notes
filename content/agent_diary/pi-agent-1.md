...
{{< admonition type="tip" title="但不一定要用到插件" >}}
...
随着我不断的使用，终于在一次会话中将DeepSeek V4 Pro的1M上下文用到了10%的程度（平时都是完成一段相关的任务后就新建会话了），第一次体验到了上下文腐化的威力。模型在这个时候已经完全忘掉了AGENTS.md中的各种指示，它不会停下来问你，编辑失败也不会去用write，而是回归它的原始记忆“sed+cat+grep”——带着错误的参数，同时还可能犯一些低级错误。

其中令我印象最深刻的一次是它把正则表达式写错了，导致匹配不到任何结果。而且在我指出这个正则表达式的匹配结果是空的情况下，它仍然看不出来这个正则表达式是有问题的：
```javascript
/^(.+?)\\((.+?)\\)$/
```

这本来是一个很简单的正则表达式。我由于懒得开编辑器，所以就想让模型来解决这个问题。然而经过前后三次问询，超过10分钟的调试（甚至调试到后端跑测试），我终于放弃了。我回溯了对话历史（pi的`/tree`还挺好用的，不过不带文件内容的回溯，所以一定要记得做好版本管理），开始自己看错误出在哪里。原来是在下面这个正则表达式中：多转义了一次。正确的正则表达式是
```javascript
/^(.+?)\((.+?)\)$/
```

为了修这个低级错误又烧了多少冤枉token......从此以后我估计不会再愿意将上下文用到超过10%了，哪怕compact估计效果也不行（上面的调试就是在compact之后进行的）。新开一个会话，让它干另外一个任务，又好好的了。
