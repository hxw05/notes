...
{{< admonition type="details" title="本笔记中包含的内容树状图（AI整理，可能不准确）" >}}
...
在看这门课程之前，得益于日常的开发实践，我能够对其中的一些内容有一个基本的粗浅了解，这些了解往往来自于开发过程中根据实际需求所做的google search以及其中一些程序自身的手册或命令行提示。例如git的一些常用指令的使用（add/commit/push/pull/fetch/remote/merge/rebase/worktree/log、`--rebase`、`--set-upstream-to/-u`）、GitHub的使用（多年间逐渐熟悉）、打包系统（例如前端工具链）、持续集成（写博客要用）、非对称加密（SSH免密码登录要用）等。
...
本课程笔记主要是对YouTube上的课程视频以及Lecture Note的个人整理，可能会省略掉一些个人觉得没必要的部分（省略得不多），并添加一些我个人在看到相关部分的联想、过去的经验和额外的理解，因此不一定准确。Lecture 1-4的笔记主要基于视频理解，没有多少Lecture Note的整理，因此会略显简略；Lecture 5、Lecture 6的笔记在当时为了方便，是用英文编写的。
