...
{{< admonition type="tip" title="What is glob / grep?" >}}
grep是shell下的一个常用的指令，用于根据内容去搜索文件或者输出的内容。这是一个非常契合当下coding agent需求的工具，它已经存在了五十多年。

glob是一个术语，在这里指的是在命令行中使用一些特定的格式来批量表达路径或操作对象的行为。在命令行中，使用`*`、`**`、`?`、`{...}`等格式可以用来代替文件命中的某些部分，使其能够“展开”，进而搭配指令完成批量的操作。

需要注意虽然用途类似，但glob与正则表达式有本质的区别。
{{< /admonition >}}
