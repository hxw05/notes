...
最好为工具引入防呆（poka-yoke）机制，从而进一步降低模型调用工具的错误概率。这里的防呆，在一定程度上也属于优化工具参数的范围——如果工具的参数足够简单或显然，模型很难犯错误。这里的一个常识是，任何涉及到路径的参数，最好都使用绝对路径，从而消除模型需要根据环境推测相对路径的额外负担。

:::tip What is 防呆？
我最初了解的防呆是在一些硬件接口的设计上，例如内存条、显卡金手指上开的孔就是一种防呆，用于避免在主板上插反；网线、MicroUSB等接口的单面可插入性也是一种防呆，对于这类单面性的接口，如果没有这种机制、两面均可插入的话，插入错误的一面可能会导致无法预料的结果（轻则软件故障，重则硬件故障乃至电路故障）。
