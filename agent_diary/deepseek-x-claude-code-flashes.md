...
|树形文档|让模型每次工作结束后生成/维护文档，并自然借助目录形成一个树形的结构相互引用|++|+|+|这种方法的效率因模型和人而异，模型生成的文档质量较差不行，使用者没有在正确的时机（不光是在工作结束之后，也要赶在上下文腐化之前）让模型更新文档也不行，使用者生成文档的提示词不够全面甚至错误（可能导致模型更新不全面或者更新错误）也不行|
