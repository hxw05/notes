...
工具集定义也是一样。默认只有四种工具，如果有需要可以再加。但一般情况下这四种工具就已经足够了，其中的bash是多功能的，有了bash就有了任何（系统原生）操作的可能。Bash is all you need.

## 默认绕过权限（YOLO by default）

pi是默认绕过权限的，这可能在安全性角度来考虑很令人惊讶，但我们需要认识到很多coding agent中的安全机制也都只是表面功夫。

并且“自主运行”和wait for approval是天然矛盾的：比起守在电脑前面逐一判断指令或操作的风险，大多数人（包括我）更想要的是放在那里让他自己干，然后开始干别的事情，过了二三十分钟之后成品直接被端上来。毕竟只有这样才能算是真正的解放了我们的（一部分）时间。不过前面这种方式存在一种让另一个LLM来担任审核的pattern。[Simon Willison的一篇博客](https://simonwillison.net/2023/Apr/25/dual-llm-pattern/)提到了他自己提出的这种双LLM的安全机制，相关笔记见于[这里](./dual-llm-pattern.md)。
