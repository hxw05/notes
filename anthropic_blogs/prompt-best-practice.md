...
我参考了当时无意间不知在哪里瞥见的一些pratice，例如role prompting，也在开头加上了role prompting，而且也确实很自然地就over role prompting了，其实我自己心里也清楚这样没有什么作用。使用Markdown是因为觉得模型对Markdown的理解没有壁垒，毕竟模型就是“说”Markdown的。
...
虽然说这篇博文对我而言不是100%的可信，因为它缺少了很多定量的验证环节，但或许大模型的黑盒性质就是需要我们像NP问题一样去试探。这篇博文集中整理了一些他们觉得的best pratice，确实是一个可用于实际的不错参考。
